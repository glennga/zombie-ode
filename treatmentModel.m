function [ ] = treatmentModel(T, dt, alpha, beta, zeta, delta, rho, c)
...
% C     = Zombie to susceptible, cure rate.
...
for j = 1:n
    s(j + 1) = max(s(j) + dt * (-beta*s(j)*z(j) + c*z(j)), 0);
    i(j + 1) = max(i(j) + dt * (beta*s(j)*z(j) - rho*i(j) - delta*i(j)), 0);
    z(j + 1) = max(z(j) + dt * (rho*i(j) + zeta*r(j) - alpha*s(j)*z(j) -c*z(j)), 0);
